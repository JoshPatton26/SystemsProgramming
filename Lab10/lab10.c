...
int main(int argc, char **argv) {
    ...
    char out[20];
    char err[20];
    sprintf(out, "%d.out", pid);
    sprintf(err, "%d.err", pid);
    ...
    if ((fdin = open(out, O_CREAT, O_RDONLY)) == -1) {
       printf("Error opening file PID.out for input\n");
       exit(-1);
    }

    /* open file to write standard output stream in append mode.
       create a new file if the file does not exist. */
    if ((fdout = open(err, O_CREAT | O_APPEND | O_WRONLY, 0755)) == -1) {
